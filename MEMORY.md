...
- CONSTRAINT: Cowork sandbox blocks api.travelpayouts.com (403); --real must run on Max's PC.
- Max's machine: `python` not on PATH, use `py` launcher (Python 3.11.5). run.bat auto-detects.
- Execution is MANUAL (Cowork scheduled task `flight-price-daily-collect` left DISABLED).
